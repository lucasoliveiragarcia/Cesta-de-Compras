...
QTD_LISTA_COMPRA = 200;
QTD_PRODUTO = 18;
QTD_COMPRA_REALIZADA = 20; #Valor maximo aleatorio por Pessoa
QTD_COMPRA = 0;
...
def gerarArquivo():
    ...
    arquivoSql.write(getScriptCompraRealizada())
...
def getScriptTipoMedida():
    sql = """
    \nINSERT INTO TipoMedida (IdTipoMedida,TipoMedida)
    VALUES  (1,'KG'),
		(2,'Litro'),
		(3,'Grama'),
		(4,'UNIDADE'),
...
def getScriptConsumidor(QTD = QTD_PESSOA):
    ...
    sql += " (1, 1, 'educouto', '123', 'Novato', '2018/02/01') "
...
def getScriptProduto(QTD = QTD_PRODUTO):
    ...
    sql = " \nINSERT INTO Produto (IdProduto, Nome, IdTipoMedida, IdMarca, unidade, valido) VALUES   "
...
def getScriptCompraRealizada(QTD = QTD_COMPRA_REALIZADA):
    sqlretorno = "\n"
    registros = 0
    idCompraRealizada = 1;
    idCompra = 1;
    ...
    for IdConsumidor in range(1,QTD_PESSOA):
        ...
        for compraCliente in range(1,compraConsumidor+1):
            ...
            sqlCompra = "INSERT INTO Compra(IdEstabelecimento ,IdProduto,IdCompraRealizada ,valido , preco) VALUES "
            sqlValuesCompra = ""

            for i in range(0,len(lstIdProdutos)):
                if(len(sqlValuesCompra) > 0):
                    sqlValuesCompra += ",\n"
                sqlValuesCompra += "({0},{1},{2},1,{3})".format(idEstabelecimento,lstIdProdutos[i],idCompraRealizada,lstValorProdutos[i]);
            ...
            sqlCompraRealizada = " INSERT INTO CompraRealizada(IdCompraRealizada,IdConsumidor,DataCompra,ValorTotal) VALUES({0},{1},'{2}',{3}) ".\
                format(idCompraRealizada,IdConsumidor,dataCompra,valorCompra)
